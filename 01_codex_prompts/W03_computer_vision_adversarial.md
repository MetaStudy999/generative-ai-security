...
## Codex 작업 지시서: Week 03 보고서 작성

## 1. 작업 대상

```text
03_weekly_reports/w03_computer_vision_adversarial/
```
...
1. Yann LeCun et al., "Gradient-Based Learning Applied to Document Recognition", Proceedings of the IEEE, 1998.
2. Apostolos Voulodimos et al., "Deep Learning for Computer Vision: A Brief Review", Computational Intelligence and Neuroscience, 2018.
3. Y. Xu et al., "Multimodal Learning With Transformers: A Survey", IEEE TPAMI, 2023.
4. Salman Khan et al., "Transformers in Vision: A Survey", ACM Computing Surveys, 2022.
5. Z. Li et al., "A Survey of Robustness and Safety of 2D and 3D Deep Learning Models Against Adversarial Attacks", ACM Computing Surveys, 2023.
...
2. 합성곱, 풀링, 특징맵, 계층적 표현
3. 컴퓨터비전 표현학습의 발전
4. Vision Transformer의 등장 배경
5. CNN과 ViT의 inductive bias 차이
6. 멀티모달 학습과 이미지-텍스트 정합
7. 2D/3D 비전 모델의 입력 구조
8. 비전 모델 평가와 일반화 문제
...
| Convolution |  |  | P01 |
| Feature Map |  |  | P01, P02 |
| Vision Transformer |  |  | P04 |
| Multimodal Learning |  |  | P03 |
| 2D/3D Robustness |  |  | P05 |
...
3. Black-box attack
4. Transfer attack
5. FGSM, PGD 등 기본 공격 개념
6. 2D 이미지 모델과 3D 비전 모델의 취약성 차이
7. Robust accuracy
8. Safety-critical vision system의 위험
...
| 관점 | 관련 위협 | 설명 |
|---|---|---|
| Confidentiality | Model query leakage |  |
| Integrity | Adversarial example |  |
| Availability | Vision model failure |  |
| Privacy | Visual data leakage |  |
| Safety | Autonomous system misclassification |  |
| Accountability | Robustness evaluation failure |  |
```
...
RQ1. CNN과 ViT는 비전 대적공격에 대해 서로 다른 취약성을 보이는가?

RQ2. 2D 이미지 기반 대적공격 평가 지표는 3D 비전 모델의 안전성 평가에도 충분한가?

RQ3. 비전 모델의 robust accuracy와 clean accuracy 간 trade-off를 어떻게 보고해야 하는가?
```
...
| 항목 | 내용 |
|---|---|
| 대상 시스템 | 이미지 분류 모델 / 비전 기반 인식 시스템 |
| 보호 자산 | 입력 이미지, 모델 파라미터, 예측 결과, 안전 판단 |
| 공격자 | 외부 입력 조작자 / 모델 질의자 |
| 공격자의 지식 | White-box / Gray-box / Black-box |
| 공격자의 능력 | 입력 이미지 변조, adversarial perturbation 생성, 모델 반복 질의 |
| 공격 경로 | 입력 이미지, 센서 데이터, 전처리 파이프라인, 추론 API |
| 공격 성공 조건 | 사람이 보기에는 정상처럼 보이나 모델이 오분류 |
| 방어자 가정 | 테스트셋 평가, 공격 샘플 평가, 로그 기록 가능 |
| 제외 범위 | 실제 자율주행 시스템 공격, 실제 물리 장비 공격 |
```
...
|---|---|---|---|---|
| Clean Accuracy | Accuracy | 정상 이미지 테스트셋 평가 | Clean test images | 기본 성능 |
| Robust Accuracy | Robust Acc. | 공격 이미지 조건에서 정확도 측정 | Adversarial images | 강건성 |
| Attack Success Rate | ASR | 공격 성공 비율 측정 | Perturbed images | 공격 효과 |
| Perturbation Size | Lp norm | 입력 변조 크기 측정 | Original/perturbed pairs | 은닉성 |
| Transferability | Transfer rate | 다른 모델에서도 공격 성공 여부 | Multiple models | Black-box 위험 |
...
`04_experiment/experiment_report.md`에는 MNIST 또는 scikit-learn digits 데이터셋 기반의 최소 실험을 설계하라.

실습 목표:

1. Clean baseline 학습
2. 간단한 adversarial perturbation 개념 실험
3. Clean accuracy와 robust accuracy 비교
4. 공격 전후 이미지 예시 그림 생성 계획
5. Confusion matrix 또는 robust accuracy 표 작성 계획
...
Dataset: MNIST 또는 scikit-learn digits
Seed: 42
```
...
| 조건 | Clean Accuracy | Robust Accuracy | ASR | 해석 |
|---|---:|---:|---:|---|
| Clean baseline |  |  |  |  |
...
|---:|---|---|---|---|---|
| 1 | CNN과 ViT의 대적공격 취약성 비교 연구 | 이미지 분류 모델 | Adversarial example | 비교분석/모의실험 | 높음 |
| 2 | 비전 모델의 clean accuracy와 robust accuracy 간 trade-off 분석 | 비전 AI 모델 | 대적공격 | 실험분석 | 높음 |
| 3 | 2D/3D 비전 모델 대적공격 평가 기준 비교 연구 | 2D/3D 비전 모델 | 입력 조작 | 문헌분석/평가체계 | 보통 |

## 2. 기말 논문에 반영할 내용

| 논문 장 | 반영 가능 내용 |
|---|---|
| 서론 | 비전 AI 모델의 안전성과 강건성 필요성 |
| 관련연구 | CNN, ViT, multimodal learning, adversarial robustness survey |
| 연구문제 | 비전 대적공격 평가 기준 |
| 연구방법 | clean/robust accuracy 비교 실험 설계 |
| 분석/실험 | 공격 전후 성능 변화 비교 |
| 보안적 함의 | 무결성, 안전성, 책임성 관점 |
| 결론 | 재현 가능한 비전 강건성 평가체계 제안 |
```
