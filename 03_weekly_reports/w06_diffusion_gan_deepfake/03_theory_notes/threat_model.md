...
| 항목 | 내용 |
|---|---|
| 대상 시스템 | 확률생성모형(Diffusion/GAN) 및 딥페이크 검출를 사용하는 ML/AI 시스템 |
...
| 공격자의 능력 | 딥페이크 생성과 악용 가능성, 합성미디어 기반 허위증거 위험, 딥페이크 탐지 모델의 일반화 문제, Cross-dataset generalization, 딥페이크 탐지의 FPR/FNR 문제 |
...
RQ1. 확률생성모형(Diffusion/GAN) 및 딥페이크 검출 생명주기에서 가장 우선적으로 보증해야 할 자산은 무엇인가?

RQ2. 딥페이크 생성과 악용 가능성, 합성미디어 기반 허위증거 위험는 어느 단계에서 발생하며 어떤 지표로 측정할 수 있는가?
