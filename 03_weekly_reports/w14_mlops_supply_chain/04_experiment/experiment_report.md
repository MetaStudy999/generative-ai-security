...
W14 실습은 MLOps/DevOps/데이터/모델 파이프라인/공급망 보안의 보안 평가를 안전한 toy 환경에서 설계하는 것이다. 실제 시스템 침해나 실제 개인정보 사용은 제외한다.
...
| 단계 | 설계 내용 | 결과 기록 |
|---|---|---|
| 간단한 ML 프로젝트 파이프라인 구조 작성 | 설계 완료 | 실제 실행 결과는 실행 후 기록 |
| 데이터 버전, 모델 버전, config, seed, 결과 로그 관리 설계 | 설계 완료 | 실제 실행 결과는 실행 후 기록 |
| 모델 아티팩트 해시 검증 설계 | 설계 완료 | 실제 실행 결과는 실행 후 기록 |
| Drift monitoring 체크리스트 작성 | 설계 완료 | 실제 실행 결과는 실행 후 기록 |
