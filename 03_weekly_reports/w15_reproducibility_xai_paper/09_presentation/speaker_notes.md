...
1분 20초. W15 실습은 모델 성능 실험이 아니라 로컬 산출물 감사라고 먼저 말한다. 47/47, 9/9, PDF 5개, 검증률 0.70, AI 고지 9/9를 `outputs/run_log.md` 기준으로 제시한다.

## 슬라이드 7. 확인 필요 항목

1분. 완성 상태와 별개로 P03 원문, P05 최종 DOI, 국내 문헌, 프레임워크 그림은 남은 리스크라고 말한다. 숨기지 않는 것이 W15의 핵심이라고 정리한다.
...
40초. 평가와 설명은 결과가 아니라 증거라는 메시지로 마무리한다. 질문을 받을 때는 DOI 미검증과 대체 PDF 상태를 먼저 투명하게 인정한다.
