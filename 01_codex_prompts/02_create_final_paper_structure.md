...
수식 또는 지표 정의가 필요한 경우에는 원문 또는 검증 가능한 표준 정의만 사용하고, 기호 설명, 직관적 의미, 보안 해석, 평가 지표 연결, 적용 한계를 함께 작성한다. 실행 로그가 없는 정량값은 `실행 전` 또는 `확인 필요`로 둔다.
...
필요한 경우에만 작성한다. 예: `ASR = N_success / N_trials`, `Leakage = N_leak / N_sensitive_tests`, `RC = N_checked_items / N_required_items`. 각 기호와 실패 조건을 함께 설명하고, 실제 개인정보 또는 무단 공격 절차는 제외한다.
...
10. 수식 또는 알고리즘은 연구문제, 위협모형, 평가방법을 이해하는 데 필요한 경우에만 포함한다.
11. 원문 확인이 부족한 수식은 `확인 필요`로 표시하고 임의로 작성하지 않는다.
12. 수식 작성은 Markdown + LaTeX math를 기본으로 하며, HTML은 KaTeX/MathJax, 문서 변환은 Pandoc, 검산은 필요 시 `sympy`를 사용한다.
13. 수식 관련 라이브러리는 즉시 일괄 설치하지 않고, 실제 검산·통계·노트북 검토가 필요할 때 선택 추가한다.
