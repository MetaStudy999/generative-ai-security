...
| 항목 | 내용 |
|---|---|
| 논문 제목 | Adversarial machine learning attacks against explainable artificial intelligence: A review |
| 저자 | G. Vadillo et al. |
| 학술지/학회 | WIREs Data Mining and Knowledge Discovery |
| 연도 | 2025 |
| DOI/URL | 확인 필요 |
| PDF 파일명 | 03_SUBSTITUTE_Baniecki_Biecek_2023_Adversarial_XAI_Survey.pdf |
| 검증 상태 | 로컬 PDF가 대체 논문 파일명으로 보임, 프롬프트 지정 논문과 대체 여부 최종 확인 필요 |

## 2. 한 문장 요약

> 이 논문은 대적방어, XAI 공격면, robustness-accuracy-fairness trade-off와 관련된 위협 분류 문제를 문헌조사와 분류체계 정리 방법으로 다루며, 공격-방어-평가 관점을 연결하는 보안 분석 틀을 기말 연구에 반영할 수 있게 해준다.
...
P03는 신경망 검증/정형방법 및 대적방어/XAI/강건성 트레이드오프 연구에서 개념 정의, 위협 분류, 평가 지표 후보를 정리하는 근거로 반영한다. 특히 공격-방어-평가 관점을 연결하는 보안 분석 틀을 관련연구와 연구방법 장에 연결한다.
