...
P03은 지정 논문 원문과 로컬 PDF가 일치하지 않고, P05는 DOI를 확인했지만 권호/issue가 필요하므로 발표에서도 남은 검토 항목으로 말한다.
...
| P03이 부분 확인이면 제출해도 되는가? | 제출용 초안에는 확인 필요 상태를 명시할 수 있지만, 최종 기말논문 확정 참고문헌에는 공식 DOI/URL과 원문 PDF가 대응되는 항목만 넣어야 한다. |
