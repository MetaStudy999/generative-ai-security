...
## Codex 작업 지시서: Week 13 보고서 작성

## 1. 작업 대상

```text
03_weekly_reports/w13_model_stealing_watermarking/
```
...
1. Daria Oliynyk et al., "I Know What You Trained Last Summer: A Survey on Stealing Machine Learning Models and Defences", ACM Computing Surveys, 2023.
2. Y. Ye et al., "A Survey of Watermarking and Fingerprinting Techniques for Deep Learning Models", ACM Computing Surveys, 2024.
3. Feng Li et al., "Deep neural network watermarking: Techniques and challenges", Neurocomputing, 2021.
4. Kaiyi Pang et al., "ModelShield: Adaptive and Robust Watermark Against Model Extraction Attack", IEEE Transactions on Information Forensics and Security, 2025.
5. Cheng Zhang et al., "Generative Adversarial Networks: A Survey on Attack and Defense", ACM Computing Surveys, 2023.
...
1. 모델 지식재산권의 개념
2. 모델 도난과 모델 추출의 차이
3. 공개 API 기반 모델 추출 구조
4. Query-response 기반 대체 모델 학습
5. Model fingerprinting
6. Model watermarking
7. DNN watermarking의 기본 구조
8. Robust watermark와 adaptive watermark
9. GAN/생성모형에서의 공격·방어 구조
10. Model utility와 protection trade-off
...
| Model IP |  |  | P01 |
| Model Stealing |  |  | P01 |
| Model Extraction |  |  | P01, P04 |
| Watermarking |  |  | P02, P03, P04 |
| Fingerprinting |  |  | P02 |
| Robust Watermark |  |  | P04 |
| GAN Attack and Defense |  |  | P05 |
...
8. Utility degradation
9. False positive / false negative in watermark detection
10. 생성모형에서의 공격·방어 가능성
...
| 관점 | 관련 위협 | 설명 |
|---|---|---|
| Confidentiality | Model parameter / behavior leakage |  |
| Integrity | Watermark removal / watermark forgery |  |
| Availability | Query abuse / API cost exhaustion |  |
| Privacy | Training data inference through stolen model |  |
| Safety | Misuse of stolen generative model |  |
| Accountability | Ownership verification failure |  |
```
...
| 항목 | 내용 |
|---|---|
| 대상 시스템 | 공개 API로 제공되는 ML/LLM/생성모형 서비스 |
| 보호 자산 | 모델 파라미터, 모델 행동, 학습된 결정경계, 워터마크, 지식재산 |
| 공격자 | 외부 질의자 / 경쟁 서비스 운영자 / 모델 추출 시도자 |
| 공격자의 지식 | Gray-box / Black-box |
| 공격자의 능력 | 반복 질의, 입력-출력 쌍 수집, 대체 모델 학습, 워터마크 회피 시도 |
| 공격 경로 | 공개 API, query-response 로그, 모델 출력, 배포 모델 |
| 공격 성공 조건 | 원 모델과 유사한 성능의 대체 모델을 획득하거나 워터마크를 제거함 |
| 방어자 가정 | 질의 제한, 로그 감사, 워터마킹, fingerprinting, 이상 질의 탐지 가능 |
| 제외 범위 | 실제 상용 API 대상 공격, 무단 대량 질의, 실제 모델 탈취 |
```
...
|---|---|---|---|---|
| Extraction Fidelity | Agreement rate | 원 모델과 추출 모델의 출력 일치율 | Query-response pairs | 모델 유사도 |
| Substitute Accuracy | Accuracy / F1 | 대체 모델의 테스트 성능 평가 | Clean test data | 공격 성능 |
| Query Cost | Number of queries | 추출에 필요한 질의 수 측정 | Query logs | 공격 비용 |
| Watermark Detectability | Detection rate | 워터마크 검출 성공률 측정 | Watermark test set | 소유권 검증 |
| False Positive Rate | FPR | 무관 모델을 워터마크 모델로 오판한 비율 | Non-watermarked models | 신뢰성 |
| Utility Loss | Accuracy drop / quality loss | 워터마크 삽입 전후 성능 비교 | Baseline/watermarked model | 효용 손실 |
| Robustness | Detection after attack | pruning, fine-tuning, extraction 후 검출 | Attacked models | 강건성 |
...
`04_experiment/experiment_report.md`에는 모델 도난·워터마킹 평가 중심의 안전한 모의 실습을 설계하라.

실습 목표:

1. 공개 API가 아닌 toy classification model을 사용한다.
2. 원 모델과 substitute model의 출력 일치율을 비교하는 구조를 설계한다.
3. Query budget에 따른 substitute accuracy 변화를 기록할 수 있도록 설계한다.
4. 워터마킹은 실제 구현보다 검출 평가표 중심으로 작성한다.
5. 모델 소유권 검증 지표를 정리한다.
...
| 조건 | Query Budget | Extraction Fidelity | Substitute Accuracy | Watermark Detection | 해석 |
|---|---:|---:|---:|---:|---|
| Baseline model | 0 |  |  |  |  |
| Substitute low query | 100 |  |  |  |  |
| Substitute medium query | 500 |  |  |  |  |
| Substitute high query | 1000 |  |  |  |  |
| Watermarked model |  |  |  |  |  |
```
...
|---:|---|---|---|---|---|
| 1 | 공개 API 기반 AI 모델의 모델 추출 위협모형과 방어 평가 연구 | ML API 서비스 | Model extraction | 위협모형/평가체계 | 높음 |
| 2 | 딥러닝 모델 워터마킹의 탐지율·위양성·품질저하 평가 기준 연구 | DNN 모델 | Watermark removal / forgery | 문헌분석/평가설계 | 높음 |
| 3 | 생성형 AI 모델의 지식재산 보호를 위한 워터마킹·핑거프린팅 프레임워크 연구 | 생성모형 | Model stealing | 프레임워크 설계 | 높음 |
...
| 관련연구 | model stealing, watermarking, fingerprinting, GAN attack/defense |
| 연구문제 | 모델 추출 위협과 소유권 검증 문제 |
| 연구방법 | toy model 기반 query-response 분석 또는 문헌기반 평가체계 |
| 분석/실험 | extraction fidelity, query cost, watermark detection 비교 |
| 보안적 함의 | 기밀성, 무결성, 가용성, 책임성 관점 |
| 결론 | 모델 IP 보호 평가 프레임워크 제안 |
```
...
9. 보고서 본문은 한글로 작성한다.
10. 실제 상용 API에 대한 모델 추출, 무단 대량 질의, 실제 모델 탈취 절차는 작성하지 않는다.
11. 실습은 toy model 또는 synthetic data 기반의 안전한 모의평가로 제한한다.
