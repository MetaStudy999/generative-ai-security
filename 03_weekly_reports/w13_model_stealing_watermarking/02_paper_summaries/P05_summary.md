...
| 항목 | 내용 |
|---|---|
| 논문 제목 | Generative Adversarial Networks: A Survey on Attack and Defense |
| 저자 | Cheng Zhang et al. |
...
> 이 논문은 Watermarking, fingerprinting, model extraction defense와 관련된 위협 분류 문제를 문헌조사와 분류체계 정리 방법으로 다루며, 공격-방어-평가 관점을 연결하는 보안 분석 틀을 기말 연구에 반영할 수 있게 해준다.
...
P05는 모델 지식재산(IP)/모델 도난/모델 추출 위협 연구에서 개념 정의, 위협 분류, 평가 지표 후보를 정리하는 근거로 반영한다. 특히 공격-방어-평가 관점을 연결하는 보안 분석 틀을 관련연구와 연구방법 장에 연결한다.
