...
| 검증 상태 | DOI 확인 완료 / 강의계획서 저자명 불일치: 공식 DOI 저자명은 `Amrin Maria Khan Adawadkar; Nilima Kulkarni`, 강의계획서 저자명은 `Aditya Adawadkar et al.` |
...
Survey 범위가 RL 전반에 넓게 퍼져 있어 DRL verification이나 reward hacking을 깊게 다루지는 않는다. W09에서는 P03/P05와 결합해 보상조작과 검증 관점을 보완한다. 공식 DOI 메타데이터는 `Amrin Maria Khan Adawadkar; Nilima Kulkarni`를 저자로 확인하지만, 강의계획서의 `Aditya Adawadkar et al.` 표기와 불일치하므로 오기인지 별도 지정 문헌을 뜻하는지는 확인 필요다.
