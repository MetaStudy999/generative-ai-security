...
Survey가 주로 기존 연구의 taxonomy를 제공하므로, 특정 모델과 데이터셋에서 방어가 얼마나 안정적인지는 별도 실험이 필요하다. 또한 vision 중심 연구가 다른 모달리티, LLM, RAG 데이터 파이프라인으로 일반화되는지는 추가 검토가 필요하다. 추가로, 강의계획서 지정 제목과 현재 로컬 PDF 제목 불일치가 있어 최종 제출 전 문헌 패킷 판본 확인이 필요하다.
