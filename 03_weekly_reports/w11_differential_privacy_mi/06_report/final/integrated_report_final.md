...
| P03 Pan et al.[3] | deep learning에서 DP 적용 위치와 보호 대상 | DOI `10.1016/j.neucom.2024.127663` 확인, 로컬 PDF는 Fu et al. 관련 논문 PDF |
| P04 Hu et al.[4] | MI attack taxonomy, confidence/loss/output signal | DOI `10.1145/3523273` 확인 |
| P05 Hu/Li Hu et al.[5] | MI defense taxonomy, utility-privacy trade-off | DOI `10.1145/3620667` 확인, 로컬 PDF는 Bai et al. 관련 논문 PDF, 강의자료 저자 표기 대조 필요 |

P03과 P05는 지정 논문과 현재 로컬 PDF가 일치하지 않는다. P03 로컬 PDF는 Fu et al.의 DP-FL systematic review이고, P05 로컬 PDF는 Bai et al.의 FL-MIA survey이다. 두 관련 논문 PDF는 W10-FL과의 연결을 설명하는 보조 문헌으로만 사용하고, 지정 논문처럼 확정 인용하지 않는다.
...
| P03 | DP 적용 위치와 보호 대상은 어떻게 달라지는가 | 지정 논문과 관련 논문 PDF 분리 | Neurocomputing 지정 논문, DP-FL 관련 논문 PDF | sample/client/update level 비교 | FL update leakage | epsilon/accounting, utility | 원문 PDF 미확보 | W10-FL 연결, 관련 논문 PDF 인용 주의 |
...
| P05 | MI 방어는 어떤 trade-off를 만드는가 | defense taxonomy | ACM 지정 논문, FL-MIA 관련 논문 PDF | output restriction, regularization, DP | output leakage, FL update leakage | clean accuracy, MI risk, utility drop | 원문 PDF 미확보, 저자 표기 대조 필요 | 방어 평가표 |
...
AI는 문헌 요약 보강, DOI/URL 검증 보조, 관련 논문 PDF 표시, synthetic DP/MI 실험 코드 작성, 발표자료 작성, KCI/SCI 섹션 보완에 사용되었다. 최종 제출자는 원고의 내용, 인용, 실험결과, 연구윤리 책임을 확인해야 한다. 상세 고지는 `05_ai_worklog/ai_disclosure_draft.md`에 둔다.
...
- 문헌 메시지: P03/P05는 관련 논문 PDF 상태이므로 원문 확보 전까지 부분 검증으로 표시한다.
...
| AI 원리 70% 정리 | 완료 |  |
| 보안 이슈 30% 정리 | 완료 |  |
| 논문 5편 요약 | 완료 | P03/P05 관련 논문 PDF 상태 반영 |
