...
| 본인 수정 내용 | 주차별 논문/관련 논문 상태 확인, 실험 수치와 outputs 대조, 안전 범위와 한계 문장 확인, 최종 제출 전 관련 보조 문헌 분리 |
...
def update_ai_worklog(week_dir: Path) -> None:
    ...
    text = textwrap.dedent(
        ...
        | 본인 수정 내용 | 문헌 검증 상태와 관련 보조 문헌 구분, 실험 수치와 outputs 대조, 안전 범위 및 한계 표현 확인 |
...
- 관련 논문 또는 관련 보조 문헌은 핵심 논문과 구분해 참고문헌 검증표 또는 한계 섹션에서 다룬다.
...
def paper_alignment_status(week_dir: Path, report_text: str) -> str:
    ...
    markers = ["표기 차이", "관련 논문", "관련 보조 문헌", "관련 논문 PDF"]
    if any(marker in check_text for marker in markers):
        return "관련 논문 재분류 항목 있음"
...
| 참고문헌 검증표 | 준비, 관련 보조 문헌 구분 있음 | `04_final_paper/06_appendices/reference_verification.md` |
...
- [ ] 관련 논문 또는 관련 보조 문헌이 핵심 논문과 구분되어 쓰이는지 확인했다.
...
def build_audit(chart_results: dict[str, tuple[str, str]], checks: list[tuple[str, int, str]]) -> None:
    ...
    remaining = [
        ...
        "일부 주차의 강의계획서 지정 논문과 주제상 의미가 맞는 문헌/PDF는 `관련 논문` 또는 `관련 보조 문헌`으로 구분했다.",
        "기말 PDF 변환본은 pandoc/libreoffice 미설치로 생성하지 못했다.",
        "관련 논문 또는 관련 보조 문헌은 본문 핵심 논문과 구분해 검증표/한계 항목에서 사용해야 한다.",
    ]
...
## 4. 참고문헌 관련 논문/보조 문헌 항목

| 주차 | 상태 |
|---|---|
{mismatch_table}

우선 검증 대상인 W01 P04, W05 P02/P04, W06 P02/P03, W07 P03/P05, W08 P01/P02/P04/P05, W09 P03/P04/P05, W11 P03/P05, W12 P01/P03/P04/P05, W13 P02/P05, W14 P01/P03/P04/P05, W15 P03/P05는 허위 DOI를 만들지 않고 기존 `doi_check.md`와 보고서 검증표에서 `논문 / 확인` 또는 `관련 논문 / 확인`으로 구분했다. 주차 주제와 의미가 같은 경우에는 관련 논문으로 정리한다.
...
- 관련 논문/관련 보조 문헌의 공식 DOI/출판사 페이지와 주차 주제 적합성.
...
| 참고문헌 검증 | {'PASS' if refs_code == 0 else '미충족'} | `04_final_paper/06_appendices/reference_verification.md` | 주차별 논문/관련 논문 구분 반영 |
...
| 2 | 주차별 관련 논문/보조 문헌 구분 | 상 | 반영 | 각 주차 `doi_check.md` 기준으로 `논문 / 확인` 또는 `관련 논문 / 확인` 표시 |
