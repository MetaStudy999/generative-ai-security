...
| 항목 | 내용 |
|---|---|
| 수식/알고리즘 이름 | DP-SGD Clipping과 Noise Addition |
| 원문 위치 | 논문 세부 절/쪽/그림/알고리즘 번호 확인 필요. 로컬 DOI/URL 점검표로 문헌 대응만 확인. |
| 작성 형식 | Markdown + LaTeX math |
| 검산 도구 | 사용 안 함 |
| 수식 또는 절차 | 표준 정의식 / 원문 직접 인용 아님.<br>$$\bar{g}_i=\frac{g_i}{\max(1,\lVert g_i\rVert_2/C)},\qquad \tilde{g}=\frac{1}{B}\left(\sum_{i=1}^{B}\bar{g}_i+\mathcal{N}(0,\sigma^2C^2I)\right)$$ |
| 기호·입력·출력 | \(g_i\): sample gradient, \(C\): clipping norm, \(B\): batch size, \(\sigma\): noise multiplier |
| 직관적 의미 | DP-SGD Clipping과 Noise Addition는 Differential Privacy·Membership Inference 평가에서 핵심 원리나 평가 지표를 정량적으로 해석하기 위한 표준식이다. |
...
| 평가 지표와 연결 | epsilon/delta accountant, accuracy, utility drop, privacy leakage |
