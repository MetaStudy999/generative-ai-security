...
## Codex 작업 지시서: Week 02 보고서 작성

## 1. 작업 대상

```text
03_weekly_reports/w02_optimization_data_poisoning/
```
...
| AI 원리 | 대규모 최적화, 효율적 학습, 압축, 경량화, 일반화         |
| 보안 이슈 | 데이터 오염, 훈련데이터 조작, Poisoning, Backdoor |
| 논문 패킷 | 5편                                    |
...
1. Léon Bottou et al., "Optimization Methods for Large-Scale Machine Learning", SIAM Review, 2018.
2. Gulzar Menghani, "Efficient Deep Learning: A Survey on Making Deep Learning Models Smaller, Faster, and Better", ACM Computing Surveys, 2023.
3. Zhipeng Tian et al., "A Comprehensive Survey on Poisoning Attacks and Countermeasures in Machine Learning", ACM Computing Surveys, 2022.
4. Antonio Cinà et al., "Training Data Poisoning Attacks and Defenses: A Systematic Review", ACM Computing Surveys, 2023.
...
4. Stochastic Gradient Descent와 변형 알고리즘
5. 최적화와 일반화의 관계
6. 모델 압축, 경량화, 효율적 학습
7. 정확도-비용-속도 트레이드오프
8. 효율화가 보안성에 미치는 영향
...
| 개념 | 정의 | 직관적 설명 | 관련 논문 |
|---|---|---|---|
| Large-scale Optimization |  |  | P01 |
| Stochastic Gradient Descent |  |  | P01 |
| Generalization |  |  | P01 |
| Model Compression |  |  | P02 |
| Efficient Deep Learning |  |  | P02 |
...
1. 데이터 오염 공격
2. Label-flipping poisoning
3. Backdoor attack
4. Training data manipulation
5. 공격자 비용모형
6. 방어 및 탐지 방법
7. Clean accuracy와 ASR의 차이
8. Stealthiness 관점
...
| 관점 | 관련 위협 | 설명 |
|---|---|---|
| Confidentiality | Training data inference |  |
| Integrity | Data poisoning / Backdoor |  |
| Availability | Model degradation |  |
| Privacy | Poisoned data source leakage |  |
| Safety | Trigger-based misclassification |  |
| Accountability | Poisoning provenance tracking |  |
```
...
RQ1. 데이터 오염 공격은 학습 데이터 비율과 공격 유형에 따라 모델 성능과 공격 성공률에 어떤 영향을 미치는가?

RQ2. Backdoor 공격은 clean accuracy를 유지하면서도 특정 trigger 조건에서 오분류를 유도할 수 있는가?

RQ3. Poisoning/Backdoor 방어평가에는 clean accuracy, ASR, stealthiness, detection rate 중 어떤 지표 조합이 필요한가?
```
...
| 보호 자산 | 학습데이터, 라벨, 모델 파라미터, 예측 결과 |
| 공격자 | 데이터 제공자 / 내부자 / 외부 데이터 오염자 |
| 공격자의 지식 | White-box / Gray-box / Black-box |
| 공격자의 능력 | 일부 학습데이터 라벨 조작, trigger 삽입, 데이터셋 오염 |
| 공격 경로 | 데이터 수집, 라벨링, 학습, 배포 |
| 공격 성공 조건 | clean accuracy 저하 또는 trigger 조건부 오분류 |
| 방어자 가정 | 데이터 검증, 실험 로그, 성능 평가 가능 |
| 제외 범위 | 실제 운영 시스템 침해, 실제 개인정보 데이터 사용 |
```
...
| Poisoning Impact | Accuracy drop | 오염률별 성능 비교 | Poisoned train data | 모델 저하 |
| Attack Success Rate | ASR | trigger 삽입 샘플 오분류율 | Triggered test data | Backdoor 효과 |
| Stealthiness | Clean acc 유지율 | clean acc와 ASR 동시 비교 | Clean/triggered data | 은닉성 |
| Detection | Detection rate | 이상치 또는 trigger 탐지 | Inspection data | 방어 평가 |
...
3. 오염률별 accuracy 비교
4. Backdoor trigger 개념 설계
5. Clean accuracy와 ASR 비교
...
|---:|---|---|---|---|---|
| 1 | 데이터 오염 공격이 ML 모델 성능과 신뢰성에 미치는 영향 분석 | 분류 모델 | Label-flipping / Poisoning | 모의실험 | 높음 |
| 2 | Backdoor 공격의 clean accuracy와 ASR 간 괴리 분석 | 이미지 분류 모델 | Backdoor | 실험분석 | 높음 |
| 3 | Poisoning/Backdoor 방어평가를 위한 다중지표 프레임워크 연구 | ML 시스템 | 데이터 오염 | 프레임워크 설계 | 높음 |
...
|---|---|
| 서론 | 데이터 오염 공격의 위험성 |
| 관련연구 | 최적화, 효율학습, poisoning, backdoor survey |
| 연구문제 | 오염률과 공격 성공률 간 관계 |
| 연구방법 | label-flipping 및 backdoor 모의실험 |
| 분석/실험 | clean accuracy, ASR, stealthiness 비교 |
