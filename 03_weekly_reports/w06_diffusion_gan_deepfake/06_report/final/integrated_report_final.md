...
보안 관점에서 FPR은 무고한 사람을 의심하게 만드는 위험이고, FNR은 실제 조작물을 놓치는 위험이다. AUROC는 threshold와 독립적인 분리력을 보여주지만, 실제 운영에서는 threshold, review band, 자동판정률, 검토율이 함께 기록되어야 한다. ECE는 detector confidence와 실제 정답률 사이의 표기 차이를 점검하는 보조 지표다.
