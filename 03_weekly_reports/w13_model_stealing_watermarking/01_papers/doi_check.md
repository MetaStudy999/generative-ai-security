...
| P02 | 지정: A Survey of Watermarking and Fingerprinting Techniques for Deep Learning Models / 로컬: Watermarking Techniques for Large Language Models: A Survey | 로컬 PDF: `https://arxiv.org/abs/2409.00089`; 지정 후보 추가 확인 메모 | 관련 논문 PDF | 로컬 PDF는 Yuqing Liang et al. arXiv:2409.00089v1. 지정 제목/저자/연도와 동일하지 않음 | Y. Ye et al. 강의자료 표기와 별개로 관련 논문으로 정리. Peigen Ye et al. ACM CSUR 2026 DOI `10.1145/3773028`은 후보일 뿐 강의자료 표기 문헌으로 관련 논문으로 분류 |
...
| P05 | 지정 후보: Generative Adversarial Networks: A Survey on Attack and Defense Perspective / 로컬: Generative Adversarial Networks: A Survey Towards Private and Secure Applications | 지정 후보 DOI `https://doi.org/10.1145/3615336`, ACM: `https://dl.acm.org/doi/10.1145/3615336`, 로컬 arXiv: `https://arxiv.org/abs/2106.03785` | 지정 후보 확인 / 로컬 관련 논문 PDF | Chenhan Zhang et al. 공식 ACM 논문은 ACM CSUR 56(4), 1-35, online 2023-11-10, print 2024-04-30. 로컬 PDF는 Zhipeng Cai et al.로 표기 차이 | 강의계획서의 `Cheng Zhang` 표기와 공식 `Chenhan Zhang` 차이 추가 확인 메모. 로컬 Cai et al. PDF를 지정 논문처럼 인용 금지 |

## 검증 원칙

1. DOI는 추측해서 적지 않는다.
2. P02/P05의 관련 논문 PDF는 최종 반영표 기준으로 관련 논문 여부를 구분한다.
3. P01/P03/P04/P05의 공식 DOI는 보고서 본문과 참고문헌에 반영하되, 강의계획서 표기 차이는 `추가 확인 메모`로 남긴다.
