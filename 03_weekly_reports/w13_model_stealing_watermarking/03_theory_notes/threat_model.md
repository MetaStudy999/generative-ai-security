...
| 항목 | 내용 |
|---|---|
| 대상 시스템 | 모델 지식재산(IP)/모델 도난/모델 추출 위협를 사용하는 ML/AI 시스템 |
...
| 공격자의 능력 | Model stealing, Model extraction, API query abuse, Substitute model training, Watermark removal |
...
RQ1. 모델 지식재산(IP)/모델 도난/모델 추출 위협 생명주기에서 가장 우선적으로 보증해야 할 자산은 무엇인가?

RQ2. Model stealing, Model extraction는 어느 단계에서 발생하며 어떤 지표로 측정할 수 있는가?
