...
HTML 슬라이드(`presentation_slides.html`)에는 모든 주차 공통으로 오른쪽 하단 슬라이드 이동 네비게이션을 넣는다. 기본 구현은 이 문서 하단의 `HTML 공통 이동 버튼 스니펫` 또는 별도 파일 `02_report_templates/presentation_slide_navigation_snippet.html`을 사용한다.

공통 요구사항:

- `.slide-nav` 클래스로 이전/다음 화살표 버튼과 `현재 / 전체` 카운터를 화면 오른쪽 하단에 고정 표시한다.
...
- 인쇄 시 `.slide-nav`는 숨긴다.
- Nature 로고, 공식 템플릿, 학술지 상표 요소는 사용하지 않는다.

Nature-style 연구 발표 체크리스트:

- 연구 질문과 핵심 주장 1개를 제목부에 명확히 둔다.
- 핵심 수식 1개 이상, 수식 기호 설명표, 직관적 의미, 보안적 의미, 평가 지표 연결, 한계를 포함한다.
- 논문 5편 역할표, 위협모형 표, 평가 프로토콜 표 중 최소 1개 이상을 포함한다.
- 수치 데이터가 있으면 `metrics_summary.csv` 근거 그래프를 넣고 데이터 출처를 적는다.
- 수치 데이터가 없으면 임의 그래프를 만들지 않고 `design_only`, `실행 전`, `확인 필요`로 표시한다.
- pipeline, threat map, evaluation flow 중 주차 주제와 직접 연결된 다이어그램을 최소 1개 포함한다.
- 결과 슬라이드는 Figure-first 구조를 사용하고 caption, panel label, limitation box를 둔다.
...
# 9. Figure-first 결과와 해석

```html
<section class="slide figure-first">
  <div class="slide-header">Key Result</div>
  <h2>Clean accuracy만으로는 보안 위험을 설명하지 못한다</h2>

  <div class="figure-grid two-panel">
    <figure class="figure-panel">
      <div class="panel-label">A</div>
      <!-- metrics_summary.csv 기반 그래프 또는 design_only placeholder -->
      <figcaption>Clean performance across conditions.</figcaption>
    </figure>

    <figure class="figure-panel">
      <div class="panel-label">B</div>
      <!-- threat model, ASR, leakage, robustness 등 주차별 핵심 지표 -->
      <figcaption>Security metric under the evaluated condition.</figcaption>
    </figure>
  </div>

  <p class="figure-caption">
    Fig. 1 | 실제 수치가 있는 경우에만 그래프를 생성한다. 수치가 없으면 실행 전으로 표시한다.
  </p>
</section>
```
