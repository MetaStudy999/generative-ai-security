...
| P05 | DRL verification과 safety specification, 단 강의계획서 `H. Yan et al.`과 현재 PDF `Landers/Doryab` 저자명 차이 확인 필요 |
...
해석: manipulated reward는 observed reward를 높게 보이게 만들 수 있지만 true security objective 기준 성능과 안전성을 악화시켰다. 이 결과는 synthetic toy cyber-defense state/action/reward simulation의 평가 형식 검증용 수치이며, 실제 IDS/IPS 제품, 실제 운영망, 실제 neural DRL policy의 보안 성능으로 일반화하지 않는다.
