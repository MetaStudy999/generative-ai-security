...
P03/P04/P05는 로컬 PDF가 관련 보조 문헌이므로 지정 논문처럼 인용하지 않는다. P01도 수업자료의 `Bayram Eken` 표기와 DOI/arXiv/로컬 PDF의 `Beyza Eken` 표기가 다르므로 최종 참고문헌에서 재확인한다.
...
4. P03/P04/P05처럼 로컬 PDF가 관련 보조 문헌일 때 보고서 인용은 어디까지 허용되는가?
...
Limitations and threats to validity는 toy logistic regression과 synthetic data, real registry/CI/CD/Kubernetes/package scanner/telemetry 미구현, simplified drift score와 evidence coverage proxy, P03/P04/P05의 로컬 관련 논문 PDF 상태, outputs와 보고서 수치 정합성 유지 필요로 정리한다.

## 14. 발표용 요약

발표 핵심 메시지는 "운영형 AI 시스템 보안은 accuracy가 아니라 evidence set을 요구한다"이다. 발표자료는 `09_presentation/`에 있으며, `presentation_slides.md`, `presentation_slides.html`, `presentation_report.md`, `speaker_notes.md`, `qna.md`, `one_page_handout.md`가 포함된다. 발표에서는 P03/P04/P05의 관련 논문 PDF 상태와 drift score 해석 한계를 반드시 언급한다.
...
| P05 지정 논문 원문/저자 표기 | 확인 필요 | 수업자료 Xiang Chen 표기와 DOI 표기 차이 |
...
![W14 metrics chart](../../09_presentation/assets/charts/w14_metrics_chart.svg)
