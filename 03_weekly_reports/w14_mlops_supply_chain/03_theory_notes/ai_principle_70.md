...
이번 실행에서는 drift score가 0.307626으로 threshold 0.25를 넘어 `drift_detected=true`가 되었다. 이 값은 synthetic 기준 분포와 drifted sample 분포 사이의 평균 표준화 feature shift이며, 공격 성공률이나 실제 운영 장애 확률이 아니다.
...
MLOps 원리는 보안통제로 바로 바뀐다. 데이터 버전관리는 데이터 오염 탐지의 기준점이고, model hash는 artifact 변조 탐지의 기준점이며, drift monitoring은 운영 중 성능·안전성 변화를 감지하는 장치다. Audit coverage와 artifact inventory는 사고대응과 책임추적을 위한 toy evidence coverage이며, 실제 기업 감사 완전성이나 완전한 AI BOM을 의미하지 않는다.
