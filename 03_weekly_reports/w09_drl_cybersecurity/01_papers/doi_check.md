...
> 검증일: 2026-06-24 KST. 검증 근거는 Crossref DOI 등록 메타데이터, DOI가 가리키는 출판사 primary URL, 기존 로컬 PDF 첫 페이지 기록이다. 공식 DOI 존재는 확인했지만, 강의계획서 지정 저자명과 공식 DOI 메타데이터가 충돌하는 항목은 `추가 확인 메모`로 유지한다.
...
| P03 | Deep Reinforcement Learning for Cyber Security | https://doi.org/10.1109/TNNLS.2021.3121870 | IEEE Transactions on Neural Networks and Learning Systems, 34(8), 3779-3795, Aug. 2023 | IEEE early access/arXiv v4 표기 | DOI 확인 완료 / 강의계획서 저자명 표기 차이 | DOI 저자 `Thanh Thi Nguyen; Vijay Janapa Reddi`와 강의계획서 `Ngoc-Tinh Nguyen et al.` 충돌 |
| P04 | Cyber-security and reinforcement learning — A brief survey | https://doi.org/10.1016/j.engappai.2022.105116 | Engineering Applications of Artificial Intelligence, 114, Article 105116, Sep. 2022 | Elsevier 출판판 PDF | DOI 확인 완료 / 강의계획서 저자명 표기 차이 | DOI 저자 `Amrin Maria Khan Adawadkar; Nilima Kulkarni`와 강의계획서 `Aditya Adawadkar et al.` 충돌 |
| P05 | Deep Reinforcement Learning Verification: A Survey | https://doi.org/10.1145/3596444 | ACM Computing Surveys, 55(14s), Article 330, pp. 1-31, online Jul. 17, 2023 | ACM 출판판 PDF | DOI 확인 완료 / 강의계획서 지정 저자 동일 여부 추가 확인 메모 | DOI 저자 `Matthew Landers; Afsaneh Doryab`와 강의계획서 `H. Yan et al.` 충돌 |
...
| P03 | DOI 확인 완료 / 강의계획서 저자명 표기 차이: 제목·학술지·연도는 강의계획서와 맞지만 첫 저자명이 다름 | `Ngoc-Tinh Nguyen` 표기가 오기인지, 별도 강의자료 표기 문헌을 뜻하는지 교수자/강의자료 원본 추가 확인 메모 |
| P04 | DOI 확인 완료 / 강의계획서 저자명 표기 차이: 제목·학술지는 맞지만 첫 저자명이 다름 | `Aditya Adawadkar` 표기가 오기인지, 별도 강의자료 표기 문헌을 뜻하는지 추가 확인 메모 |
| P05 | DOI 확인 완료 / 강의계획서 지정 저자 동일 여부 추가 확인 메모: 제목·학술지는 맞지만 저자명이 완전히 다름 | `H. Yan et al.` 지정 논문과 Landers/Doryab 문헌의 동일 여부 추가 확인 메모 |

주의: W09의 P05는 강의계획서 지정 저자명 `H. Yan et al.`과 현재 로컬 PDF 기준 `Matthew Landers; Afsaneh Doryab`가 다르므로, 동일 논문 여부와 최종 ACM 출판정보를 추가 확인 메모 상태로 유지한다.
...
3. 강의계획서 저자명과 DOI/PDF 저자명이 충돌하는 항목은 `추가 확인 메모`로 유지한다.
