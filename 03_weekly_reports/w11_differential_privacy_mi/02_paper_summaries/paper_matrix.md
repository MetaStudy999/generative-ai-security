...
| P03 | DP deep learning에서 보호 대상과 적용 위치는 어떻게 달라지는가 | 지정 논문과 로컬 관련 논문 PDF 분리 필요 | 지정 논문은 Neurocomputing, 로컬 관련 논문 PDF는 DP-FL systematic review | centralized/deep/FL DP 적용 위치 비교 | sample-level, client-level, update-level leakage | epsilon/accounting, utility, FL privacy metric | 지정 논문 PDF 미확보, 강의자료의 저자명 표기 확인 필요, 관련 논문 PDF 상태 | W10-FL과 W11-DP 연결, 단 관련 논문 PDF를 지정 논문처럼 인용 금지 |
...
| P05 | MI 방어는 어떤 utility-privacy trade-off를 만드는가 | MI defense taxonomy survey, 로컬 관련 논문 PDF 분리 필요 | 지정 논문은 ACM CSUR, 로컬 관련 논문 PDF는 FL-MIA survey | output restriction, regularization, DP, calibration 등 방어축 | output leakage, overfitting, FL update leakage | clean accuracy, MI risk, utility drop | DOI `10.1145/3620667` 확인, 지정 논문 PDF 미확보, 강의자료의 저자명 표기 확인 필요 | 방어 평가표와 보안적 함의, 단 관련 논문 PDF를 지정 논문처럼 인용 금지 |
...
P01은 DP claim 자체의 오용을 비판하는 기준점이다. P02는 중앙집중형 deep learning에서 DP-SGD, auditing, utility-privacy trade-off를 체계화한다. P03은 지정 논문과 로컬 관련 논문 PDF가 달라서 deep learning DP와 federated DP를 분리해 다뤄야 한다. P04는 MI 공격 taxonomy를 제공하고, P05는 방어 taxonomy와 utility-privacy trade-off를 설명한다.
...
기여 후보는 “AI 보안 연구에서 privacy claim을 검증하기 위한 최소 보고항목: accuracy, train accuracy, MI attack accuracy, privacy leakage score, utility drop, epsilon/accounting, noise setting, seed/config/output log”로 정리할 수 있다. P03/P05는 원문 확보 전까지 관련 논문 PDF로만 표시하고, 지정 논문처럼 확정 인용하지 않는다.
