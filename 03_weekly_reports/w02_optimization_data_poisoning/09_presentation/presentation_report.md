...
| P02 | Efficient Deep Learning | 압축, 경량화, 비용-속도-품질 trade-off 설명 | ACM DOI `10.1145/3578938` 확인, Article 번호 확인 필요 |
...
| P04 | Wild Patterns Reloaded | training data poisoning threat model과 방어 관점 | DOI `10.1145/3585385` 확인, 강의계획서 지정 제목과 동일 여부 확인 필요 |
...
| Safe toy backdoor | 5% | 0.970370 | 0.970359 | 0.987654 | 정상 성능을 유지하면서 trigger 조건에서 실패 |

주의: safe toy backdoor 결과는 안전한 축소 모형의 지표 분리 예시이며, 실제 공격 성능이나 실서비스 취약성으로 일반화하지 않는다.
...
| □ | P02 DOI 확인 결과와 P04 강의계획서 제목 표기 차이/확인 필요성을 숨기지 않는다. |
| □ | 예상 질문 3개 이상을 준비했다. |
