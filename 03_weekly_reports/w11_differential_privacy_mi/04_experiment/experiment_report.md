...
W11 실습은 차등프라이버시(DP) 및 멤버십 추론 공격/방어의 보안 평가를 안전한 toy 환경에서 설계하는 것이다. 실제 시스템 침해나 실제 개인정보 사용은 제외한다.
...
| 단계 | 설계 내용 | 결과 기록 |
|---|---|---|
| 공개 데이터 또는 synthetic data 기반 baseline 모델 학습 | 설계 완료 | 실제 실행 결과는 실행 후 기록 |
| Overfitting 정도에 따른 MI 위험 개념 설명 | 설계 완료 | 실제 실행 결과는 실행 후 기록 |
| DP-like noise 또는 regularization 적용 전후 비교 설계 | 설계 완료 | 실제 실행 결과는 실행 후 기록 |
| Privacy budget과 accuracy trade-off 표 작성 | 설계 완료 | 실제 실행 결과는 실행 후 기록 |
