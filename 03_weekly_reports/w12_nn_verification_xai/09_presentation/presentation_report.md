...
| ID | 역할 |
|---|---|
| P01 | Abstraction 기반 신경망 검증의 배경. 로컬 PDF는 `RELATED` |
| P02 | 대적공격과 방어의 taxonomy. DOI/원문 대조 필요 |
| P03 | XAI 설명 결과가 공격면이 될 수 있다는 근거. 로컬 PDF는 `RELATED` |
...
P01, P03, P04, P05는 로컬 PDF가 지정 논문과 다르거나 `RELATED` 상태다. P02도 강의 표기와 공식 후보 DOI 사이 차이가 있어 최종 제출 전 공식 출판 페이지와 원문 PDF를 사람이 재확인해야 한다. 공개 저장소에 학술 PDF를 포함하기 전 저작권과 공개 범위를 별도로 점검한다.
