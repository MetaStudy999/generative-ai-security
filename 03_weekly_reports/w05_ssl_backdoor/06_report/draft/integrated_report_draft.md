...
| ID | 논문 | 저자 | 연도 | 검증 상태 |
|---|---|---|---|---|
| P01 | A Survey on Self-Supervised Learning: Algorithms, Applications, and Future Trends | Jie Gui et al. | 2024 | TPAMI DOI `10.1109/TPAMI.2024.3415112` 확인, 강의계획서 저자 표기 확인 필요 |
| P02 | A Comprehensive Survey on Self-Supervised Learning for Recommendation | Xubin Ren et al. | 2025 | DOI 확인, 지정 일반 SSL survey 동일 여부 확인 필요 |
| P03 | Self-Supervised Learning for Videos: A Survey | Madeline C. Schiappa et al. | 2023 | DOI 확인, Article 번호 확인 필요 |
| P04 | Threats to Training: A Survey of Poisoning Attacks and Defenses on Machine Learning Systems | Zhibo Wang et al. | 2022 | DOI 확인, 강의계획서 제목/저자 표기 확인 필요 |
| P05 | A survey of backdoor attacks and defences: From deep neural networks to large language models | Ling-Xin Jin et al. | 2025 | DOI 확인, 강의계획서 저자 표기 확인 필요 |
...
P02 지정 문헌 동일 여부, P03 Article 번호, P04 제목/저자 표기, P05 강의계획서 저자 표기는 최종 확인이 필요하다. 또한 survey 문헌의 분류체계를 toy 실험과 어떻게 연결할지 추가 검토가 필요하다.

## 7. 실습 요약

실습은 synthetic 2D representation 기반으로 실행했다. 실제 개인정보, 실제 서비스 침해, 무단 질의, 악용 가능한 절차는 포함하지 않는다. clean accuracy는 1.000000, poisoned clean accuracy는 1.000000, triggered source embedding ASR은 1.000000, consistency defense 후 ASR은 0.000000이다. 이 값은 toy 평가 지표 설명용이며 실제 SSL/파운데이션 모델 보안 성능으로 일반화하지 않는다.
