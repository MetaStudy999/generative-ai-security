...
## Codex 작업 지시서: Week 07 보고서 작성

## 1. 작업 대상

```text
03_weekly_reports/w07_llm_security_privacy/
```
...
2. Ankur Das et al., "Security and Privacy Challenges of Large Language Models: A Survey", ACM Computing Surveys, 2025.
3. Mingzhe Yao et al., "A survey on large language model security and privacy: problems, methods, and opportunities", AI Open, 2024.
4. Yongtao Yin et al., "A survey on multimodal large language models", National Science Review, 2024.
5. Shujun Li et al., "When Software Security Meets Large Language Models: A Survey", IEEE/CAA Journal of Automatica Sinica, 2025.
...
1. LLM의 기본 구조
2. Pretraining, instruction tuning, alignment
3. RLHF와 preference optimization 개념
4. LLM 추론 구조
5. Context window와 tokenization
6. LLM 평가 벤치마크
7. Benchmark contamination
8. Multimodal LLM의 구조
9. LLM 평가의 재현성 문제
...
| 개념 | 정의 | 직관적 설명 | 관련 논문 |
|---|---|---|---|
| Large Language Model |  |  | P01, P02 |
| Pretraining |  |  | P01 |
| Alignment |  |  | P01 |
| Evaluation Benchmark |  |  | P01 |
| Benchmark Contamination |  |  | P01 |
| Multimodal LLM |  |  | P04 |
| Software Security Interface |  |  | P05 |
...
7. Model inversion 또는 memorization 위험
8. LLM 기반 코드 생성과 소프트웨어 보안 취약점
9. Benchmark contamination과 evaluation leakage
10. LLM evaluation reproducibility
...
| 관점 | 관련 위협 | 설명 |
|---|---|---|
| Confidentiality | Data extraction / prompt leakage |  |
| Integrity | Prompt injection / jailbreak |  |
| Availability | LLM service abuse / denial of wallet |  |
| Privacy | Sensitive information disclosure |  |
| Safety | Harmful generation / insecure code |  |
| Accountability | Evaluation contamination / audit failure |  |
```
...
RQ1. LLM 평가 벤치마크의 오염은 모델 성능 해석과 보안성 평가에 어떤 영향을 미치는가?

RQ2. LLM 시스템에서 데이터 추출, 프롬프트 공격, 소프트웨어 보안 취약점은 어떤 공통 공격면을 갖는가?

RQ3. LLM 보안 평가를 위해 attack success, privacy leakage, utility, reproducibility를 함께 측정하는 프로토콜은 어떻게 설계해야 하는가?
```
...
| 항목 | 내용 |
|---|---|
| 대상 시스템 | LLM 기반 질의응답 시스템 / 코드 생성 시스템 / 멀티모달 LLM |
| 보호 자산 | 학습데이터, 시스템 프롬프트, 사용자 프롬프트, 모델 응답, 코드 산출물 |
| 공격자 | 외부 사용자 / 악성 프롬프트 작성자 / 내부 로그 접근자 / 모델 질의자 |
| 공격자의 지식 | White-box / Gray-box / Black-box |
| 공격자의 능력 | 프롬프트 조작, 반복 질의, 민감정보 유도, 코드 생성 요청 |
| 공격 경로 | 사용자 입력, 시스템 프롬프트, API, 로그, 코드 생성 결과 |
| 공격 성공 조건 | 민감정보 노출, 정책 우회, 취약 코드 생성, 평가 오염 |
| 방어자 가정 | 입력 필터링, 로그 감사, 출력 검증, 평가 데이터 관리 가능 |
| 제외 범위 | 실제 민감정보 추출 시도, 실제 서비스 공격, 불법적 우회 |
```
...
|---|---|---|---|---|
| Utility | Task score | 정상 질의 성능 평가 | Clean prompts | 기본 성능 |
| Attack Success Rate | ASR | 공격 프롬프트 성공률 | Safe synthetic prompts | 공격 효과 |
| Privacy Leakage | Leakage score | 민감정보 노출 여부 측정 | Synthetic sensitive prompts | 프라이버시 |
| Refusal Quality | Refusal accuracy | 위험 요청 거절 여부 평가 | Safety prompts | 안전성 |
| Code Security | Vulnerability rate | 생성 코드 취약점 분석 | Code prompts | SW 보안 |
| Benchmark Contamination | Contamination rate | 평가 데이터 중복·노출 확인 | Benchmark metadata | 평가 신뢰성 |
...
`04_experiment/experiment_report.md`에는 LLM 보안 평가 프로토콜 중심의 안전한 모의 실습을 설계하라.

실습 목표:

1. LLM 보안 체크리스트 설계
2. Prompt leakage 모의 사례 분석
3. Jailbreak 또는 prompt injection은 공격 재현이 아니라 방어 평가 관점으로 정리
4. LLM 기반 보안 로그 분석의 환각 위험 평가 설계
5. 코드 생성 AI의 취약 코드 생성 위험 평가표 작성
...
Dataset: synthetic prompts / 공개 예제
Seed: 42
```
...
```bash
docker build -t w07-aisec .
docker run --rm -it -v $(pwd):/workspace w07-aisec bash
...
| 조건 | Utility | ASR | Privacy Leakage | Refusal Quality | 해석 |
|---|---:|---:|---:|---:|---|
| Clean prompts |  |  |  |  |  |
| Prompt attack simulation |  |  |  |  |  |
| Privacy-risk prompts |  |  |  |  |  |
...
|---:|---|---|---|---|---|
| 1 | LLM 보안·프라이버시 평가를 위한 다중지표 프레임워크 연구 | LLM 시스템 | 데이터 추출·프롬프트 공격 | 평가체계 설계 | 높음 |
| 2 | LLM 기반 보안 로그 분석 시스템의 환각 위험과 검증 구조 연구 | 보안 로그 분석 LLM | Hallucination / false analysis | 사례분석/체크리스트 | 높음 |
| 3 | 생성형 AI 코드 보조도구의 취약 코드 생성 위험 평가 연구 | Code LLM | Insecure code generation | 모의실험/평가표 | 높음 |

## 2. 기말 논문에 반영할 내용

| 논문 장 | 반영 가능 내용 |
|---|---|
| 서론 | LLM 보안·프라이버시 위험과 평가 필요성 |
| 관련연구 | LLM evaluation, LLM security/privacy, multimodal LLM, software security survey |
| 연구문제 | LLM 보안 평가 프로토콜 |
| 연구방법 | 문헌분석, synthetic prompt 평가, 체크리스트 |
| 분석/실험 | ASR, privacy leakage, utility, refusal quality 비교 |
| 보안적 함의 | 기밀성, 무결성, 프라이버시, 안전성, 책임성 |
| 결론 | 재현 가능한 LLM 보안 평가체계 제안 |
```
...
9. 보고서 본문은 한글로 작성한다.
10. 실제 민감정보 추출, 실제 서비스 공격, 불법적 우회 절차는 작성하지 않는다.
11. 공격 내용은 안전한 모의 평가와 방어 설계 관점으로만 작성한다.
